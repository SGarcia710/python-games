...
class InstruccionesNivelDos:
  def __init__(self, window):
    ...
    self.labeluno = Label (self.rootWindow, text = "recuadro derecho una consonante. El juego consiste en", bg = "white")
    self.labeluno.config(font=("Arial", 10))
    self.labeluno.pack()
    self.labeluno.place(anchor=W, x = 2, y = 140)
    self.labeldos = Label (self.rootWindow, text = "presionar el botón [Z] cuando aparezca una consonante en", bg = "white")
    self.labeldos.config(font=("Arial", 10))
    self.labeldos.pack()
    self.labeldos.place(anchor=W, x = 2, y = 160)
    self.labeltres = Label (self.rootWindow, text = "el lado derecho y el botón [-] cuando aparezca una vocal", bg = "white")
    self.labeltres.config(font=("Arial", 10))
    self.labeltres.pack()
    self.labeltres.place(anchor=W, x = 2, y = 180)
    self.labelseis = Label (self.rootWindow, text = "en el lado izquierdo. Si la letra aparece en el lado contrario", bg = "white")
    self.labelseis.config(font=("Arial", 10))
    self.labelseis.pack()
    self.labelseis.place(anchor=W, x = 2, y = 200)
    self.labelsiete = Label (self.rootWindow, text = "se debe esperar (4) segundos sin presionar ningun boton, de lo", bg = "white")
    self.labelsiete.config(font=("Arial", 10))
    self.labelsiete.pack()
    self.labelsiete.place(anchor=W, x = 2, y = 220)
    self.labelocho = Label (self.rootWindow, text = "contrario sera contado como un error.", bg = "white")
